...
func test_field_defaults() -> void:
	...
	assert_eq(b.mode, AttributeBonus.MODE_ADD, "mode defaults to MODE_ADD")
	assert_eq(b.value, 0.0, "value defaults to 0.0")
...
func test_field_round_trip() -> void:
	...
	assert_eq(b.mode, AttributeBonus.MODE_ADD, "mode round-trip")
	assert_eq(b.value, 4.0, "value round-trip")
...
func test_mode_constants_are_distinct_stringnames() -> void:
	var modes: Array[StringName] = [
		AttributeBonusScript.MODE_ADD,
		AttributeBonusScript.MODE_MULTIPLY,
		AttributeBonusScript.MODE_SET,
	]
	for i in modes.size():
...
func test_bonus_value_can_be_negative() -> void:
	var b: AttributeBonus = AttributeBonusScript.new()
	b.value = -2.0
	assert_eq(b.value, -2.0, "value can be negative (penalty)")
