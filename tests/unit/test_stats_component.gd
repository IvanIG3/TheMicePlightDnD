...
const StatsComponentScript := preload("res://scripts/stats_component.gd")
const StatsBaseDataScript := preload("res://scripts/stats_base_data.gd")
...
func _make_attr(con_val: int = 10, int_val: int = 10, dex_val: int = 10) -> Resource:
...
func _make_stats_base(max_hp_base: int = 10, max_energy_base: int = 3) -> Resource:
	var sb: Resource = StatsBaseDataScript.new()
...
func test_level_up_chains_until_threshold_not_met() -> void:
	_stats.init(_make_stats_base())
	# level=1, xp_to_next=100, xp=0 → 550 xp
	# 1→2 at 100, 2→3 at 300 (100 + 200 cumulative), 3→4 at 600 (100 + 200 + 300)
	# 550 < 600 → level 3, xp 250 (550 - 300)
	_stats.gain_xp(550)
...
func test_spend_energy_caps_at_max() -> void:
	...
	watch_signals(_stats)
	_stats.gain_energy(5)  # would push to 7; capped at 3
...
func test_set_initiative_from_dex_uses_modifier() -> void:
	...
	# Per spec table: modifier = get_value - 10 (linear, NOT D&D 5e /(2) formula).
	# DEX 16 → mod 6. (Spec test scenario says "mod 3" using D&D formula; we
	# follow the spec table's linear formula because that's the locked code path.)
