...
- **Code style (Godot 4 / GDScript)** — based on the [official GDScript style guide](https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/gdscript_styleguide.html). Apply these on every new or modified `.gd` file.
  ...
  **Language and design**
  ...
  - **Finite value sets in a constants class.** When a `StringName` field has a closed set of valid values (damage types, attributes, modes, etc.), define them in a dedicated `XxxTypes` / `XxxIds` class with `ALL: Array[StringName]`. The class is the single source of truth: executors compare against its constants, tests reference it, and the canonical list of valid values lives there. To add an inspector dropdown, override `_validate_property()` and inject `PROPERTY_HINT_ENUM` with `",".join(PackedStringArray(Xxx.ALL))` — in Godot 4.7.1 this does not render the dropdown for `TYPE_STRING_NAME` (verified empirically), so it is currently omitted. Minimum test in the constants file's test suite: `test_no_duplicates_in_all`. Reference: `effect/damage_types.gd`, `attribute/attribute_ids.gd`.
  ...
  - **Prefer `class_name` over `preload` for script references.** When a script has a `class_name`, use `X.new()` instead of `const XScript := preload("res://path/x.gd")` + `XScript.new()`. The class_name is path-independent, so moving a script between directories does not break consumers. Exceptions: scripts without a `class_name` (e.g. autoloads that shadow their own class, like `TurnManager`); and `.tscn` resources, which have no class_name mechanism and must be preloaded.
